...
2. **`profile`** — `"default"` (full HVG cap, 30 PCs, 15 expression neighbors, 6 spatial neighbors) vs **`"fast"`** (1000 HVG, 20 PCs, 10 neighbors, 4 spatial neighbors) for quicker dev/UI runs. Validator: `--fast`. Leiden uses the **igraph** backend when `igraph` is installed (`ml/requirements-spatial.txt`).
3. **Converter** — `visium_hd_square_to_h5ad.py` supports `--max-spots` (same caveat: full MTX is still read first).
4. **Benchmark metadata** — Sprint 4 can attach `cohort_summary` from `.h5ad` using **backed** reads (`h5ad_metadata_summary`) to avoid loading `X`.
...
For **large** dev files, combine **`--fast`** with a moderate **`--max-obs`** so validation finishes in minutes instead of hours:
...
| Date | Machine | n_obs (file) | profile | max_obs | Sprint 1 s | Sprint 2 s | Peak RSS note |
|------|---------|----------------|---------|---------|------------|------------|----------------|
| 2026-04-11 | macOS 15.7.3, arm64 (local); `square_016um` dev `.h5ad` (`build_dev_h5ad.sh`, 50k spots, filter-in-tissue, no histology) | 50000 | fast | 10000 | 26.0 | 2.1 | ~1.9 GB `ru_maxrss` after run (macOS reports bytes) |
| _fill in_ | | | default | | | | |
